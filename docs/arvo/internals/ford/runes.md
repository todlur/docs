...
`+faslus` in your dojo. This example is a generator. For more information on generators, see the [generator docs](https://urbit.org/docs/arvo/generators).
```
/+  time-to-id
::
:-  %say
...
Another feature of the `/+ and `/-` runes is the ability to specify the
`ship` and `case` from which to load the library.
